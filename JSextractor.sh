...
check_deps() {
  ...
  local tools=(subfinder httpx gau waybackurls katana gospider hakrawler jq curl python3)
  local missing=()
...
phase_subdomains() {
  ...
  echo "$DOMAIN" > "$OUTDIR/subdomains/subs.txt"
  if have subfinder; then
    subfinder -d "$DOMAIN" -silent 2>>"$OUTDIR/logs/subfinder.log" >> "$OUTDIR/subdomains/subs.txt"
  fi
  sort -u -o "$OUTDIR/subdomains/subs.txt" "$OUTDIR/subdomains/subs.txt"

  if have httpx; then
    httpx -silent -l "$OUTDIR/subdomains/subs.txt" -o "$OUTDIR/subdomains/live_hosts.txt" \
      2>>"$OUTDIR/logs/httpx.log"
  else
    sed 's#^#https://#' "$OUTDIR/subdomains/subs.txt" > "$OUTDIR/subdomains/live_hosts.txt"
  fi
  log "Live hosts: $(wc -l < "$OUTDIR/subdomains/live_hosts.txt")"
}
...
download_pending() {
  ...
  # source map grab reveals original file tree
